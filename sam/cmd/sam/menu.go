...
// TUI attaches to tmux itself (suspending and resuming via tea.ExecProcess)
// and adding a workspace is an in-TUI form, so it stays open until the user
// quits.
//
// The menu is also the sole interactive entry into first-run setup: when no
// config exists yet, the TUI opens with the add-workspace form already
// active (the CLI verbs stay non-interactive and error instead — see
// loadWorkspaceAndConfig). A cancelled first run quits the TUI cleanly
...
func runMenu(deps tui.Deps, start tui.Resource) error {
	...
	if firstRun {
		// No config yet: an empty workspace set with default TUI settings.
		// The form's save writes the file; until then nothing exists.
		cfg = &config.Config{Workspaces: map[string]config.Workspace{}, Tui: config.DefaultTui()}
	...
	return tui.Run(name, ws, cfg.Workspaces, start, firstRun, cfg.Tui, deps)
}
